...
\begin{align}
    ...
    m_K &= \frac{4}{3}\pi r^3 \rho_K = \qty{0.4523(28)}{\kilo\gram}
\end{align}
...
\begin{align}
    g_{diff} &= \lvert g_\text{phys} - g_\text{mat}\rvert\\ = \qty{5.16(3)e-2}{\meter\per\second\squared}
...
\begin{resultbox}[Ergebnis Aufgabe 2]
    ...
    \begin{equation}
    g_{diff} = \qty{5.16(3)e-2}{\meter\per\second\squared} \Rightarrow 2.27\sigma  
    \end{equation}
\end{resultbox}

\bigskip
\section{Diskussion der Messwerte}
Die berechneten Messwerte des mathematischen und physikalischen Pendels weisen durch die hohe Anzahl gemessener Schwingungen einen sehr kleinen statistischen Fehler auf ($g_\text{mat} = \qty{10.782(16)}{\meter\per\second\squared}$). Dennoch fällt auf, dass die relative Abweichung vom Literaturwert ca. $9.91\%$ beträgt. Aufgrund der kleinen Unsicherheit resultiert diese Abweichung in einer  Abweichung von rund $60\sigma$. Diese riesige Sigma-Abweichung zeigt, dass hier kein Statistischer Fehler vorliegt, sondern ein massiver systematischer Fehler in der Messung oder Auswertung die Ergebnisse verfälscht. Eine Begründung durch nicht-berücksichtigte physikalische Effekte ist nicht möglich, da sämtliche relevanten Korrekturterme in der Theorie abgedeckt wurden. Die Ursache für dieser starken systematischen Abweichung muss daher in den Messwerten liegen. 
...
\begin{align}
    g_\text{mat} &= \qty{9.893(16)}{\meter\per\second\squared} \Rightarrow 8.75\sigma \\
    g_\text{phys} &= \qty{9.949(16)}{\meter\per\second\squared} \Rightarrow 5.22\sigma
\end{align}

was einer relativen Abweichung von $1.42\%$ vom Literaturwert entspricht und den sigma-Fehler reduziert. Somit ist die Messung des mathematischen Pendels und des physikalischen Pendels nun konsistent(er) mit dem Literaturwert.
