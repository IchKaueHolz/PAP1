...
\begin{equation}
    \omega_0 = \frac{2\pi}{T_0} = \frac{2\pi}{\qty{1.89(22)}{\second}} = \qty{3.3(4)}{\per\second}
\end{equation}
 
Mit der Dämpfungskonstante $\delta = \qty{2.06(6)e-4}{\per\second}$ ergibt sich somit für den Korrekturterm:
\begin{equation}
    k_4 = \frac{\delta^2}{\omega_0^2} = \frac{\qty{2.06(6)e-4}{\per\second}^2}{\qty{3.3(4)}{\per\second}^2} = \num{3.9(9)e-9}
\end{equation}
...
\begin{equation}
    k_{ges} = 1 + \sum_{i=1}^{5}{k_i} = 1.0004203(14)
\end{equation}

Somit ergibt sich für die Schwerebeschleunigung:
\begin{align}
...
\begin{resultbox}[Ergebnis Aufgabe 2]
    Die Schwerebeschleunigung beträgt nach Berücksichtigung sämtlicher Korrekturterme:
    \begin{equation}
...
Die Messwerte des Mathematischen Pendels und des Physikalischen Pendels stimmen miteinander überein. Die penible Berücksichtigung sämtlicher Korrekturterme hat den Wert für die Schwerebeschleunigung $g$ nur geringfügig erhöht. Die Abweichung vom Literaturwert beträgt aufgrund des großen Fehlers in beiden Fällen weniger als $0.5\sigma$, was auf eine übereinstimmende Messung hindeutet, jedoch fällt auf, dass die realtive Abweichung vom Literaturwert ca. $9.23\%$ beträgt. Das deutet auf einen systematischn Fehler hin. Eine Begründung durch nicht-berücksichtigte Korrekturterme ist nicht möglich, da sämtliche relevanten Effekte berücksichtigt wurden. Möliche Ursachen für die Abweichung könnten in der Messung der Fadenlänge oder der Schwingungsdauer liegen.
...
\begin{align}
    g_\text{mat} &= \qty{9.89(2.21)}{\meter\per\second\squared}\\
    g_\text{phys} &= \qty{9.90(2.21)}{\meter\per\second\squared}
\end{align}

was einer relativen Abweichung von $0.9\%$ vom Literaturwert entspricht. Und den Sigma fehler von $g_\text{phys}$ auf $0.04\sigma$ reduziert. Somit ist die Messung des mathematischen Pendels und des physikalischen Pendels nun konsistent(er) mit dem Literaturwert.

\paragraph{Korrekturterm}
Des Weiteren fällt die Insignifikanz des Korrekturterms auf. Alle Korrekturterme sind kleiner, als der Fehler der Messung des mathematischen Pendels. Somit ist die Berücksichtigung sämtlicher Korrekturterme für die Bestimmung von $g$ nicht notwendig, da der Fehler der Messung den Korrekturterm überwiegt. Es kann aber auch daran liegen, dass wir mit der falschen Fadenlänge auch einen zu 
kleine Anzahl der zu messenden Schwingungen $n$ gewählt haben, was den Fehler der Zeitmessung verändert.
Sehr bemerkenswert ist, dass die Dämpfung eine so kleine Korrektur aufweisst
