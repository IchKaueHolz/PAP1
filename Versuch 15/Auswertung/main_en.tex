...
The Moment of inertia $I$ for a solid cylinder is $I = \frac{1}{2}mr^2$, and for a hollow cylinder, it is $I = \frac{1}{2}m(r_{out}^2 + r_{in}^2)$
...
with the measured height $h = \qty{10.1(4)e-2}{\text{m}}$\footnote{The height was measured two days later, because the initial measurement was not accurate.} and the length of the track $l = \qty{1.000(5)}{\text{m}}$, the angle of inclination can be calculated as:
\begin{align}
    \varphi &= \arcsin\left(\frac{h}{L}\right) = \arcsin\left(\frac{\num{10.1(4)e-2}}{\num{1.000(5)}}\right) \\
    &\approx \num{5.797(24)}^\circ = \qty{0.1012(4)}{\text{rad}}
\end{align}
...
\begin{align}
    a_{solid} &= \frac{g \sin\varphi}{1 + \frac{I_{solid}}{m r^2}} = \qty{0.661(26)}{\meter\per\second\squared} \\
    a_{hollow} &= \frac{g \sin\varphi}{1 + \frac{I_{hollow}}{m r^2}} = \qty{0.513(21)}{\meter\per\second\squared}
\end{align}
...
\begin{align}
    a_\text{solid, exp} &= \qty{0.617(4)}{\meter\per\second\squared} \\
    a_\text{hollow, exp} &= \qty{0.4907(27)}{\meter\per\second\squared}
\end{align}
...
\begin{align}
    \sigma_\text{solid} &= \frac{|a_\text{solid, exp} - a_\text{solid, theo}|}{\sigma_\text{hollow, theo}} \Rightarrow 1.73 \sigma \\
    \sigma_\text{hollow} &= \frac{|a_\text{hollow, exp} - a_\text{hollow, theo}|}{\sigma_\text{solid, theo}} \Rightarrow 1.19\sigma
\end{align}
...
\begin{equation}
    h_{eff} = \frac{h}{l} \cdot \left(l-11\si{\milli\meter}\right) = \qty{0.100(4)}{\meter}
\end{equation}

The potential energy for the cylinders equals:
\begin{equation}
    E_{pot} = (\num{0.443(1)})\,\si{\kilo\gram} \cdot 9.81\,\si{\meter\per\second\squared} \cdot \qty{0.100(4)}{\meter} = \qty{0.434(18)}{\joule}
\end{equation}
...
\begin{table}[H]
    \centering
    \caption{Measured times and calculated speeds for the solid and hollow cylinders.}
...
\begin{equation}
    E_{kin} = \frac{1}{2}mv^2 + \frac{1}{2}I\omega^2
\end{equation}


This results in the following kinetic energies for the solid and hollow cylinders:
...
Comparing the total kinetic energies with the initial potential energy, we get:
\begin{align}
    E_{kin, solid} &= \qty{0.351(4)}{\joule} \\
    E_{kin, hollow} &= \qty{0.367(4)}{\joule} \\
    E_{pot} &= \qty{0.434(18)}{\joule}
\end{align}

This results in a deviation of:
\begin{align}
    \Delta E_{solid} &= E_{pot} - E_{kin, solid} = \qty{0.083(18)}{\joule}\\
    & \Rightarrow 4.61\sigma \\
    \Delta E_{hollow} &= E_{pot} - E_{kin, hollow} = \qty{0.067(18)}{\joule}\\
    & \Rightarrow 3.72\sigma
\end{align}

\begin{resultbox}[Conclusion]
    The total kinetic energies for both cylinders are lower than the initial potential energy, indicating that some energy was lost during the rolling motion:
        \begin{center}
        \centering
        \begin{tabular}{lcc}
            \toprule
...
In figure \ref{fig:acceleration_comparison} you can see that the acceleration of the solid cylinder is higher than that of the hollow cylinder, which is consistent with the theoretical predictions. This is due to the fact that the solid cylinder has a lower moment of inertia, allowing it to accelerate more quickly down the incline.

Even though the experimental results show the expected trend, there are significant deviations from the theoretical values. The experimental accelerations for both cylinders are lower than the theoretical predictions. The kinetic energies show deviations from the theoretical predictions as well.
The lower experimental accelerations suggest that additional resistive forces, such as rolling friction may have influenced the motion of the cylinders. 
The total kinetic energies for both cylinders are lower than the initial potential energy, indicating that some energy was lost during the rolling motion, likely due to friction.

Since friction is proportional to the normal force, which is in turn proportional to the mass of the rolling body, it is expected that the solid cylinder would experience a greater frictional force than the hollow cylinder. This could explain why the solid cylinder has a lower kinetic energy than the hollow cylinder, despite having a higher acceleration, as well as bigger deviations from the theoretical predictions, which neglect resistive forces. The solid cylinder's greater mass results in a larger normal force, leading to increased frictional losses during its motion down the incline.

It is very unlikely that the timing and measurement errors are the main contributors to the observed deviations, as the experimental setup was carefully calibrated and repeated measurements were taken to minimize random errors. I suggest that the measurement of the height difference should be repeated, as it was measured two days later and could have been affected by systematic errors, because the height difference that, was measured in the beginnung was a lot higher than the one measured two days later. Identifying and measuring the height of the slope was not very precise, because holding the caliper perpendicularly to the surface was difficult. Determining the geometry of the ramp more accurately could help to reduce the discrepancies between the theoretical and experimental results.

\section{Conclusion}
All in all, the experiment successfully demonstrated the principles of rolling motion and energy conservation, while also highlighting the importance of considering real-world factors such as friction and measurement accuracy in experimental physics.
It was a lot of fun to do this experiment, and I learned a lot about the physics of rolling motion and the importance of careful measurement and analysis in experimental physics. I also learned how to use the precise light barrier system and the stopwatch system, which was very interesting.
