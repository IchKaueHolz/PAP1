...
with the measured height $h = \num{12.2(4)e-2}\,\text{m}$ and the length of the track $l = \num{1.000(5)}\,\text{m}$, the angle of inclination can be calculated as:
\begin{align}
...
\begin{align}
    a_{solid} &= \frac{g \sin\varphi}{1 + \frac{I_{solid}}{m r^2}} = \num{0.798(26)}\\
    a_{hollow} &= \frac{g \sin\varphi}{1 + \frac{I_{hollow}}{m r^2}} = \num{0.619(21)}
\end{align}
...
For the height difference $h$, we can use the following relation:

\begin{equation}
    h = sin\varphi \cdot \left(l-11\si{\milli\meter}\right) = \num{0.121(4)e-2}\,\si{\meter}
\end{equation}

The potential energy for the three cylinders are:
\begin{table}
    \centering
    \caption{Potential energy of the cylinders at the top of the ramp.}
    \label{tab:potential_energy}
    \begin{tabular}{lcc}
        \toprule
        \textbf{Cylinder Type} & \textbf{Mass [g]} & \textbf{Potential Energy [J]} \\
        \midrule
    \end{tabular}
\end{table}
