...
Somit ergibt sich für $k'$ mit Gleichung \ref{eq:log10I}(vorzeichen) der Wert:
\begin{equation}
    k' =(\num{0.121(18)})\si{\per\centi\meter}
\end{equation}


Die verwendeten Küvetten besitzten eine Konzentration von $c = \SI{0.000001}{\mole\per\liter}$.
...
Die Abweichung der beiden Messmethoden von nur $0.03\sigma$ beweißt die Konstenz der Messmethodik. Es ist zu erkennen, dass die Abweichung der Messwerte zum Literaturwert bei beiden Messmethoden größer als $1\sigma$ beträgt. Dies zeigt, dass die Messmethoden zwar konsistent sind, aber systematische Fehler in der Versuchsdurchführung oder in der Kalibrierung des Messgeräts vorliegen könnten. Mögliche Ursachen für diese Abweichungen könnten ungenaue Pipettierungen, Schwankungen in der Lichtquelle oder unzureichende Berücksichtigung von Streulicht sein.

Ausserdem befindet sich die Messwellenlänge bei $\lambda = \SI{629}{\nano\meter}$, während der Literaturwert bei $\lambda = \SI{625}{\nano\meter}$ angegeben ist. Dies könnte ebenfalls zu einer Abweichung führen, da der molare Extinktionskoeffizient wellenlängenabhängig ist.

\medskip
Ein Hinweis auf dem Datenblatt des FCF-Farbstoffs ist \cite{PAPskript}:
\begin{quote}
    \emph{Loss on Drying: $\leq 13\%$}\\
    \emph{In H$_2$O soluble matter: $\leq 1\%$}
\end{quote}

Dies bedeutet, dass die Konzentration der Lösung um bis zu $13\%$ abweichen kann, was ebenfalls eine signifikante Fehlerquelle darstellt, da das Pulver hygroskopisch ist und Feuchtigkeit aus der Luft aufnehmen kann. Dies könnte die tatsächliche Konzentration der Lösung beeinflussen und somit die Messungen verfälschen.

Somit sind $\approx 14\%$ der beim Erstellen der Lösung gewogenen Masse kein Farbstoff, sondern Wassser oder andere Verunreinigungen. Somit ist die angegebe ne Konzentration von $\SI{5e-6}{\mole\per\liter}$ nicht korrekt. Dies könnte die Abweichung der Messwerte zum Literaturwert erklären, da die tatsächliche Konzentration der Lösung niedriger ist als angenommen, was zu einem höheren berechneten Extinktionskoeffizienten führt.
